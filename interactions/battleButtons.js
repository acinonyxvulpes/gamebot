...
export default async function handleBattleButtons(interaction) {
    ...
    if (!battle) return interaction.reply({ content: "No active battle.", ephemeral: true });
    ...
    if (interaction.customId === "battle_accept") {
        ...
        return interaction.update({
            content: `Battle started!\n\n` +
                     `${battle.player1.pet.emoji} **${battle.player1.pet.name || "Pet 1"}** vs ` +
                     `${battle.player2.pet.emoji} **${battle.player2.pet.name || "Pet 2"}**\n\n` +
    ...
    if (interaction.customId === "battle_decline") {
        endBattle(interaction.channelId);
    ...
    if (interaction.customId.startsWith("battle_")) {
        ...
        if (interaction.customId === "battle_attack") {
            ...
            if (battle.p1hp <= 0 || battle.p2hp <= 0) {
                ...
                const loser = battle.p1hp > 0 ? battle.player2 : battle.player1;
                ...
                return interaction.update({
                    content: `🎉 **${winner.pet.name} wins the battle!**\n` +
                             `+20 XP\n+50 coins`,
                    components: []
                });
            }
        ...
        if (interaction.customId === "battle_defend") {
            ...
            if (turn === "player1") battle.player1.defending = true;
            else battle.player2.defending = true;
